...
export default function LegacyLayout(props: ParentProps) {
  ...
  createEffect(() => {
    if (!desktop()) return
    void platform.setWindowRoute?.(location.pathname)
  })
  ...
  const useSDKNotificationToasts = () =>
    onMount(() => {
      ...
      const unsub = serverSDK().event.listen(async (e) => {
        ...
        const claimID = `${serverSDK().scope}\u0000${e.details.type}\u0000${directory}\u0000${props.id}`
        if ((await platform.claimNotification?.(claimID)) === false) return
  ...
  command.register("layout", () => {
    const commands: CommandOption[] = [
      ...
      ...(desktop() && platform.runDesktopMenuAction
        ? [
            {
              id: "window.new",
          ...
          <div class="size-full relative overflow-x-hidden">
            ...
            <Show when={layout.sidebar.opened()}>
              <div
                class={
                  desktop()
                    ? "block absolute inset-y-0 z-30 w-0 overflow-visible"
                    : "hidden xl:block absolute inset-y-0 z-30 w-0 overflow-visible"
                }
                style={{ left: `${side()}px` }}
            ...
            <div
              class={
                desktop()
                  ? "block pointer-events-none absolute top-0 right-0 z-0 border-t border-border-weaker-base"
            ...
            >
              <main
                classList={{
                  "size-full overflow-x-hidden flex flex-col items-start contain-strict border-t border-border-weak-base bg-background-base":
                    true,
