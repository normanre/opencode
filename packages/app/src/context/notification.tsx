...
function createServerNotificationState(input: {
  ...
  const handleSessionIdle = (directory: string, event: { properties: { sessionID?: string } }, time: number) => {
    ...
    void lookup(directory, sessionID).then((session) => {
      ...
      const claimID = `${serverSDK().scope}\u0000session.idle\u0000${directory}\u0000${sessionID}\u0000${session.version}`
  ...
  const handleSessionError = (
    ...
    void lookup(directory, sessionID).then((session) => {
      ...
      const claimID = `${serverSDK().scope}\u0000session.error\u0000${directory}\u0000${sessionID ?? "global"}\u0000${
        session?.version ?? session?.time.updated ?? time
      }`
      Promise.resolve(platform.claimNotification?.(claimID)).then((claimed) => {
        if (claimed === false || meta.disposed) return
